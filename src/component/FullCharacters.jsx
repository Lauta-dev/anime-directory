  ...
  return (
    <>
      {data.map(({ character }) => {
        ...
        return (
          <Link to={characterPaths({ name, id })} className="character_all_conteiner" key={id}>
            <b>{name}</b>
            <LazyLoadImage src={webp} width='100px' alt={`Image from ${name} of ${type}`} />
          </Link>
        )
      })}
    </>
...
function get() {
  ...
  useEffect(() => {
    async function getCharacters() {
      try {
        const fetching = await fetch(JIKAN_API_CHARACTERS({ type: isAnime, id: extracId }))
        if (!fetching.ok) return setCharacters({ error: 'Error al obtener los datos' })
...
export default function FullCharacters() {
  ...
  return (
    <section className="character_conteiner">
      {characters.error ? JSON.stringify(characters.error) : getData({ data: characters, type: isAnime, id: extracId })}
    </section>
  )
}
